...
from salary_switch.transformation.experience_band_cleaner import (
    clean_experience_band,
)
from salary_switch.transformation.position_experience_aggregation import (
    aggregate_position_experience_salaries,
)
...
def run_position_experience_pipeline(
    input_path: Path,
    output_path: Path,
) -> None:
    ...
    experience_cleaned_dataframe = clean_experience_band(
    ...
    aggregated_dataframe = aggregate_position_experience_salaries(
        converted_dataframe
    )
    ...
    print(
        "Position-experience pipeline completed: "
        f"{output_path}"
    )


if __name__ == "__main__":
    run_position_experience_pipeline(
        input_path=Path("data/raw/salary_2026.xlsx"),
        output_path=Path(
            "data/processed/position_experience_salary_summary.csv"
        ),
    )